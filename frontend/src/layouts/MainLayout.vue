  ...
  <a-layout style="min-height: 100vh">
    ...
    >
      ...
      >
        ...
        <a-menu-item key="/viz/data-screens">
          <template #icon>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
...
watch(() => route.path, (path) => {
  if (path.startsWith('/viz/dashboards')) {
    selectedKeys.value = ['/viz/dashboards']
  } else if (path.startsWith('/viz/data-screens')) {
    selectedKeys.value = ['/viz/data-screens']
  } else if (path.startsWith('/metadata')) {
    selectedKeys.value = ['/metadata/datasources']
  } else if (path.startsWith('/dataset')) {
    selectedKeys.value = ['/dataset/list']
  } else {
...
const pageTitle = computed(() => {
  if (route.path.startsWith('/viz/dashboards')) return '仪表板'
  if (route.path.startsWith('/viz/data-screens')) return '数据大屏'
  if (route.path.startsWith('/metadata')) return '数据源管理'
  if (route.path.startsWith('/dataset')) return '数据集管理'
  return '智能对话'
})
