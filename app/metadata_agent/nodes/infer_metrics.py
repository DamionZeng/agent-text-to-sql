...
async def infer_metrics(state: MetaAgentState, runtime: Runtime[MetaAgentContext]) -> dict:
    ...
    writer({"type": "progress", "step": "infer_metrics", "status": "running", "message": "AI 正在推断业务指标..."})
    ...
    if not fact_tables or not measure_columns:
        writer({"type": "progress", "step": "infer_metrics", "status": "success", "message": "未发现事实表或度量字段，跳过指标推断"})
    ...
    try:
        ...
        if not isinstance(metric_configs, list):
            metric_configs = []
        writer({"type": "progress", "step": "infer_metrics", "status": "success", "message": f"推断出 {len(metric_configs)} 个业务指标"})
        ...
        writer({"type": "progress", "step": "infer_metrics", "status": "error", "message": str(e)})
        metric_configs = []
