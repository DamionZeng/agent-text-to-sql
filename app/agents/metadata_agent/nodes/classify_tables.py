...
async def classify_tables(state: MetaAgentState, runtime: Runtime[MetaAgentContext]) -> dict:
    ...
    writer({"type": "progress", "step": "识别表类型", "status": "running", "message": "AI 正在分析表类型..."})
    ...
    try:
        ...
        writer({"type": "progress", "step": "识别表类型", "status": "success", "message": f"识别到 {dim_count} 张维度表，{fact_count} 张事实表"})
        ...
        writer({"type": "progress", "step": "识别表类型", "status": "error", "message": str(e)})
        return {
