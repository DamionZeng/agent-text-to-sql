...
async def recall_value(state: DataAgentState, runtime: Runtime[DataAgentContext]):
    ...
    writer({"type": "progress", "step": "召回值", "status": "running"})
    try:
        query = state['query']
        ...
        writer({"type": "progress", "step": "召回值", "status": "success"})
        return {"retrieved_value_infos": retrieved_value_infos}
    except Exception as e:
        logger.error(f"召回值失败: {str(e)}")
        writer({"type": "progress", "step": "召回值", "status": "error"})
        raise e